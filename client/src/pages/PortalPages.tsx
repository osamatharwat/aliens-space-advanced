...
import { claimGuestCertificate, getAnalyticsSummary, redeemCommitteeAccessCode, submitApplication } from "@/services/operations";
...
export function WorkspacePage() {
  ...
  const [workspaceData, setWorkspaceData] = useState<Record<string, { members: any[]; tasks: any[]; resources: any[]; announcements: any[] }>>({});
  ...
  const load = async () => {
    ...
    const { data, error: membershipError } = await supabase.from("committee_memberships").select("id,committee_id,status,position,committee:committees(name,slug)").eq("user_id", auth.user.id).eq("status", "active");
    if (membershipError) { setWorkspaceError(membershipError.message); setMemberships([]); return; }
    const activeMemberships = data ?? [];
    setMemberships(activeMemberships);
    const entries = await Promise.all(activeMemberships.map(async (membership: any) => {
      const [members, tasks, resources, announcements] = await Promise.all([
        supabase.from("committee_memberships").select("id,position,status,user:profiles(id,name,username,avatar_url)").eq("committee_id", membership.committee_id).eq("status", "active").order("position"),
        supabase.from("committee_tasks").select("id,title,is_completed,assigned_to,due_at").eq("committee_id", membership.committee_id).order("created_at", { ascending: false }),
        supabase.from("committee_resources").select("id,title,object_key,object_url,created_at").eq("committee_id", membership.committee_id).order("created_at", { ascending: false }),
        supabase.from("committee_announcements").select("id,title,body,created_at").eq("committee_id", membership.committee_id).order("created_at", { ascending: false }),
      ]);
      const failed = [members, tasks, resources, announcements].find((result) => result.error);
      if (failed?.error) setWorkspaceError(failed.error.message);
      return [membership.id, { members: members.data ?? [], tasks: tasks.data ?? [], resources: resources.data ?? [], announcements: announcements.data ?? [] }] as const;
    }));
    setWorkspaceData(Object.fromEntries(entries));
  };
  ...
  return <PortalShell title="Your workspace" eyebrow="Team operating system"><div className="workspace-top"><div><span className="card-kicker">Scope-aware access</span><h2>Work only where you belong.</h2><p>Every committee, task, resource, and announcement is filtered by the membership returned from Supabase.</p></div><form className="code-form" onSubmit={redeem}><Label htmlFor="access-code">Committee access code</Label><div><Input id="access-code" value={code} onChange={(event) => setCode(event.target.value)} placeholder="Enter a code" required /><Button type="submit" className="bg-emerald-300 text-slate-950 hover:bg-emerald-200"><KeyRound className="mr-2 h-4 w-4" />Redeem</Button></div></form></div>{notice && <p className="success-note">{notice}</p>}{error && <p className="error-note">{error}</p>}{workspaceError && <p className="error-note">Workspace data source error: {workspaceError}</p>}<div className="portal-grid workspace-grid">{memberships.length === 0 ? <div className="portal-empty span-full"><Database className="h-6 w-6 text-emerald-300" /><div><h2>No active committee membership.</h2><p>Registered accounts see no operational workspace until the atomic access-code flow creates an active membership.</p></div></div> : memberships.map((membership) => <section key={membership.id} className="portal-card"><div className="committee-icon"><Orbit className="h-5 w-5" /></div><span className="card-kicker">{membership.committee?.slug}</span><h2>{membership.committee?.name}</h2><Badge className="role-badge">{membership.position}</Badge><div className="workspace-links"><span><ClipboardCheck className="h-4 w-4" />Tasks <b>{workspaceData[membership.id]?.tasks.length ?? 0}</b></span><span><FolderLock className="h-4 w-4" />Resources <b>{workspaceData[membership.id]?.resources.length ?? 0}</b></span><span><UsersRound className="h-4 w-4" />Members <b>{workspaceData[membership.id]?.members.length ?? 0}</b></span></div><div className="workspace-details"><div><span className="card-kicker">Members</span>{(workspaceData[membership.id]?.members ?? []).slice(0, 4).map((member: any) => <p key={member.id}>{member.user?.name || member.user?.username || "Member"} · {member.position}</p>)}{(workspaceData[membership.id]?.members.length ?? 0) === 0 && <p className="module-note">No active members returned.</p>}</div><div><span className="card-kicker">Recent activity</span>{(workspaceData[membership.id]?.announcements ?? []).slice(0, 2).map((announcement: any) => <p key={announcement.id}>{announcement.title}</p>)}{(workspaceData[membership.id]?.tasks ?? []).slice(0, 2).map((task: any) => <p key={task.id}>{task.is_completed ? "Done" : "Open"} · {task.title}</p>)}{(workspaceData[membership.id]?.announcements.length ?? 0) + (workspaceData[membership.id]?.tasks.length ?? 0) === 0 && <p className="module-note">No recent activity returned.</p>}</div></div></section>)}</div></PortalShell>;
}
...
export function DashboardPage() {
  ...
  const [analytics, setAnalytics] = useState<{ applications: number; registrations: number; attendance: number; certificates: number; active_memberships: number } | null>(null);
  const [loading, setLoading] = useState(false);
  const [analyticsError, setAnalyticsError] = useState<string | null>(null);
  useEffect(() => { if (!auth.user) return; setLoading(true); setAnalyticsError(null); Promise.all([getAnalyticsSummary(), supabase.from("events").select("id", { count: "exact", head: true })]).then(([summary, events]) => { setAnalytics(summary); setCounts({ applications: summary.applications, events: events.count ?? 0, committee_memberships: summary.active_memberships, certificates: summary.certificates }); }).catch((reason: any) => setAnalyticsError(reason.message ?? "Analytics are unavailable for this scope.")).finally(() => setLoading(false)); }, [auth.user]);
  if (auth.loading) return <div className="portal-loading"><Loader2 className="animate-spin" />Restoring your Supabase session…</div>;
  ...
  return <PortalShell title="Command center" eyebrow="Restricted operations"><div className="dashboard-banner"><div><span className="card-kicker">Live Supabase view</span><h2>Operational truth, without theater.</h2><p>These counts are read from the connected database. Empty or unavailable tables are shown as empty—not simulated.</p></div><Badge className="live-badge">{loading ? "SYNCING" : "CONNECTED"}</Badge></div>{analyticsError && <p className="error-note">{analyticsError}</p>}<div className="metric-grid">{metrics.map(({ key, label, icon: Icon }) => 
<div className="metric-card" key={key}><Icon className="h-5 w-5 text-emerald-300" /><span>{label}</span><strong>{counts[key] ?? 0}</strong></div>)}</div>{analytics && <div className="metric-grid analytics-secondary"><div className="metric-card"><span>Registrations</span><strong>{analytics.registrations}</strong></div><div className="metric-card"><span>Attendance</span><strong>{analytics.attendance}</strong></div></div>}<div className="dashboard-modules"><Link href="/workspace" className="portal-card module-card"><KeyRound className="h-5 w-5 text-emerald-300" /><h3>Membership & access</h3><p>Activate and inspect your scope through database-enforced flows.</p><ArrowUpRight className="module-arrow h-4 w-4" /></Link><div className="portal-card module-card"><Database className="h-5 w-5 text-emerald-300" /><h3>Operational modules</h3><p>Applications, IR, events, attendance, certificates, and exports are structured for scoped procedures.</p><span className="module-note">{analytics ? "Analytics RPC returned a live summary." : "Connect the canonical migration to unlock live actions."}</span></div></div></PortalShell>;
}
